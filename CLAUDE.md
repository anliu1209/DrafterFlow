...
There is also a **browser UI** — the DrafterFlow landing page plus the tool — served by `server.py`, a thin FastAPI layer over the same pipeline with a static frontend (`static/`, Three.js 3D preview). The UI is English by default with a Chinese toggle, and light-first with a dark-mode toggle. The user plans to migrate this UI to a hosted web app later, so the transport layer is deliberately decoupled from the conversion code.
...
- **TODO (browser/WASM no-backend port)**: move the whole pipeline (image_processing → vectorize → model_builder → export) into the browser so it runs fully client-side behind Cloudflare Pages (zero backend). Library mapping: `opencv.js` (or canvas) for image processing, `potrace` (JS npm) for vectorization, `@manifold-3d/manifold` (WASM, same boolean engine trimesh uses) + `earcut` + manual binary-STL for the 3D/model. Keep the Python version as reference to verify hole centres / white-bg masks. Phase it: transparent PNG first, then white-background. Note: the 3D preview uses Three.js **WebGL** (not WebGPU); current compute is server-side CPU.
