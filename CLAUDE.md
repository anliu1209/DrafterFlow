...
- **`server.py`** — two JSON/binary endpoints:
  - `POST /api/analyze` (multipart `file` + `dark_threshold`/`alpha_threshold` + optional `base_color`/`color_color` hex, defaults `#ffffff`/`#2563eb`) → `{ ok, w_px, h_px, base_png, color_png, combined_png, dark_ratio, clearance_disabled, base_poly }`. The `base_png` is the **filled** base plate (mirrors `vectorize.base_polygons`, not the raw stroke mask); `color_png` is the dark art; `combined_png` overlays them; `base_poly` is the base-exterior rings (pixel space) the frontend uses to judge whether a placed ring is printable. `clearance_disabled` = `dark_ratio >= 0.85` (matches `model_builder`'s color-clearance cutoff).
  - `POST /api/generate` (multipart `file` + `width`/`base`/`color`/`hole`/thresholds + optional `holes` JSON `[[x,y,outer],...]`, or the single-hole `hole_x`/`hole_y`/`tab_outer_radius`) → binary STL with `X-Hole-Center: "x,y"` header. Multi-hole builds a tab (挂耳) at each hole whose `outer > hole_radius`, then cuts all inner holes.
...
- **TODO (non-transparent input)**: the pipeline currently only accepts **transparent-background RGBA PNGs** (`extract_masks` rejects non-alpha / fully-opaque images). Add support for non-transparent / white-background images — e.g. auto-remove a near-white/large flat background to alpha (or a background-color eyedropper in the UI), so white-background line art and photos can be fed in.
