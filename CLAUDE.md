...
- **`vectorize.py`** — converts each binary mask into `shapely` polygons. Self-intersecting contours (common in dense line art) are normalized via `make_valid` and flattened back to Polygon parts (`_polygons_of`), and every ring is smoothed with Chaikin corner-cutting (2 passes) to remove the pixel staircase. `base_polygons()` fills outer contours into a solid plate; `color_polygons()` preserves holes/nesting. Despite the module name and "SVG" references in DEVDOC, this is the **OpenCV-contours fallback, not potrace/SVG** — the output is shapely polygons, never SVG.
...
- The 3D preview uses **flat shading** (not smooth normals) so the 90° base/relief edge stays crisp; smooth normals rounded the relief into a "blob". Jaggies on curved silhouettes are a geometry-level issue, tracked as a TODO in `vectorize.py` — the likely next fix is Gaussian-blurring the mask before contour extraction (adds a `scipy` dependency), pending the user's go-ahead.
